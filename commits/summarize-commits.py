...
def main():
    ...
    for year, packages in year_totals.items():
         tops = dict(sorted(packages.items(), key=operator.itemgetter(1), reverse=True)[:25])
    ...
    charts = [{"divid": "top25", "labels": list(top25.keys()), "vals": list(top25.values()), "title": "Top 25 Packages by Commits over all time", "min": minval, "max": maxval}]

    for year, packages in top25years.items():
        maxval = max(packages.values())
        minval = min(packages.values())
        charts.append({"divid": "top25_%s" % year, "labels": list(packages.keys()), "vals": list(packages.values()), "title": "Top 25 Packages by commits for %s" % year, "min": minval, "max": maxval})
